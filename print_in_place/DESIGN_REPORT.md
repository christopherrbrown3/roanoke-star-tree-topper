...
Coplanar triangulation was canonicalized through Manifold, returned to the live Blender scene, and checked without increasing intersection tolerances. The exported STLs contain that cleanup. The downloadable Blender file is an earlier construction checkpoint; its final save and agreement with the current STLs have not been verified. The [scene/export status](scene_export_validation.json) records this distinction.

The 3MF is reopened programmatically to check its geometry, one-object structure, two filament definitions, and part assignments. Bambu Studio displayed two project filaments during native import. Sliced-layer verification remains incomplete; the installed command-line slicer crashed before producing a toolpath. The [release status](RELEASE_STATUS.json) records the scope of these checks.
...
The editable Blender construction checkpoint and current material meshes are alongside this report. Use the STL or 3MF exports for printing. The [rebuild guide](../scripts/REBUILD.md) describes construction, export, packaging, and validation. [Research sources](../research/SOURCES.md), the [angle worksheet](../research/ANGLE_WORKSHEET.md), and the [archive search](../research/PLAN_SEARCH.md) document the evidence and its limits.
