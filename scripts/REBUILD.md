...
6. Inspect all five images. Open the 3MF as a project in Bambu Studio and confirm filament 1 on the dark body and filament 2 on the white face. Slice and inspect the first five layers and the socket roof. Update the reports and `print_in_place/RELEASE_STATUS.json` to match the evidence. Execute `scripts/finalize_a1_blender.py` in the GUI console using the same `__file__` pattern. It checks that scene exports match the validated STLs, embeds current reports, license, and scripts, and saves an upright front view.
